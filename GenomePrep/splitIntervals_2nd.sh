...
#SBATCH --job-name="split_intervals_2nd"
...
## and moves the old list from first round to 'intervalLists/split_1st/',
## and stores new lists in 'intervalLists/split_2nd/';
...
mkdir -p intervalLists/split_2nd
mkdir -p intervalLists/split_1st
cd intervalLists/split_2nd

## set start counter
CNT=172

for ((i=${CNT}; i<=199; i++)); do
    ...
    ## move new lists to split_2nd, and old list to split_1st
    outcnt=`ls tmp | wc -l`
...
mv -n $HOME/Style/Pax_genome/intervals/intervalLists/split_2nd/*scattered.interval_list $HOME/Style/Pax_genome/intervals/intervalLists/
## ---------------------------------------------

## !! for some reason the very last output interval list contains
##    too many intervals -> run 'splitIntervals_3rd.sh'
